...
impl SearchEngine {
    ...
    /// Rebuild the vector cache if it is holding more than the table does.
    ///
...
impl SearchIndex for SearchEngine {
    ...
    async fn embed_pending(&self, batch: u32) -> Result<u32> {
        ...
        // until the server was restarted — the count was right again
        // afterwards precisely because startup rebuilds the cache from the
        // table.
        //
        // Checked here rather than pushed from the store: the store maintains
        // the lexical indexes transactionally and deliberately knows nothing
        // about this one. The table is the truth and the cache is a mirror, so
