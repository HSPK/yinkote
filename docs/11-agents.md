...
## `read_paper`：只有这个库知道的东西

`get_item` 返回的是**编目记录**——任何一个书目数据库都有的东西。一个只能看到摘要的助手，
掌握的信息和搜索引擎没有区别。

个人文献库真正值得被问的，是别的部分：**用户自己写的笔记、划的段落、以及这篇论文站在什么上面**。
`read_paper` 把这些一次给出。

一个容易写错的细节：**标注挂在附件上，不挂在论文上**——一条高亮属于某个具体文件，
而一篇论文可能有好几个文件。只翻论文自己的子条目，一条高亮都找不到。

节流方式：笔记和高亮各截断到 600 字符、各最多 40 条；参考文献只给**数量**不给全文——
一份参考文献是一百行，而它很少是当下要回答的那个问题。

## 详情面板的请求要防抖

方向键在列表里走，每秒会换 20 次选中项。详情面板为选中项取的东西（相关对话、参考文献）
于是也每秒发 20 次——每个请求都很快，但它们和列表查询争同一个连接池，而且除了最后一个
之外全部作废。

和搜索框从第一版起就防抖是同一个道理：**一次按键不是一个请求**。
首个值立即生效（等待会让打开一篇论文显得比实际慢），之后 120ms。

## `list_references`：一份可以据以行动的清单

`read_paper` 只给参考文献的**数量**，因为一百行的书目很少是当下要回答的问题。
但当它就是那个问题时——"这篇站在什么上面而我还没读过？"——需要的是清单，不是数字。

三个设计点：
- **保留原始编号**。参考文献在印出来的论文里是有编号的，重新编号是无声的破坏。
- **给出 key 而不只是"有/没有"**。模型拿到 key 就能直接去看那篇论文，
  而不是拿着刚给它的标题再搜一遍。
- **`held: no` 过滤**。这正是打开一份参考文献最常见的理由。
  但 `total` 仍然描述这篇论文，不描述过滤结果。

上限 120 条，并且**明说 `truncated`**：一篇综述能引四百篇，而拿到 400 中的 120 条的模型，
没有任何办法看出自己只看到了一部分。

这个工具是被一条**代码注释**逼出来的——上一轮我在 `read_paper` 里写了"`list_references`
在需要时就在那里"，而它并不在。注释指向不存在的东西，比没有注释更糟。
